    ...
    const blackjackService = {
        ...
        async doubleDown(username, tableId = 'default') {
            const table = this.getTable(tableId);
            const player = table.players[table.currentPlayerIndex];

            if (!player || player.username !== username || player.hand.length !== 2) return;

            const user = await User.findOne({ where: { username } });
            if (!user || user.balance < player.bet) return;

            user.balance -= player.bet;
            await user.save();

            await Transaction.create({ userId: user.id, balanceChange: -player.bet, type: 'double' });
        ...
        async split(username, tableId = 'default') {
            const table = this.getTable(tableId);
            const player = table.players[table.currentPlayerIndex];

            if (!player || player.username !== username || player.hand.length !== 2 || this.cardValue(player.hand[0]) !== this.cardValue(player.hand[1])) return;
            ...
            await Transaction.create({ userId: user.id, balanceChange: -player.bet, type: 'split' });
            ...
            player.hand.push(this.drawCard(tableId));
            player.splitHand.push(this.drawCard(tableId));

            player.activeHand = 'main'; 
        },
